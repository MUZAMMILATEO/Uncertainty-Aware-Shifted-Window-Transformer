...
class UncertaintyLoss(nn.Module):
    ...
    def forward(self, uncertainty, y_true):
        # Compute the L2 loss between predicted and actual uncertainties
        loss = F.mse_loss(uncertainty[0], uncertainty[1])
        # Add a penalty term for high uncertainties
        penalty = torch.mean(uncertainty[0])
        return loss + penalty
...
class KLDivergenceLoss(nn.Module):
    ...
    def forward(self, parameterVal, y_true):                                          # parameterVal = mu, logvar # y_true is a placeholder
        # Compute KL divergence: 0.5 * sum(1 + log(sigma^2) - mu^2 - sigma^2)
        kl_loss = -0.5 * torch.mean(1 + parameterVal[1] - parameterVal[0].pow(2) - parameterVal[1].exp())
        return kl_loss
